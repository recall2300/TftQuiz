...
export default function QuizClient({ category, count }: Props) {
  ...
  const multiplier = getComboMultiplier(state.combo + 1);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        <div className="flex items-center gap-3">
          {state.combo >= 3 && (
            <div className="flex items-center gap-1">
              ...
              <span className="font-bold text-yellow-400">{state.combo}콤보</span>
              {comboLabel && (
                <Badge className="bg-yellow-400/20 text-yellow-300 text-xs border-yellow-400/30">
                  {comboLabel}
                </Badge>
              )}
            </div>
      ...
      <div className="mb-6 flex justify-center">
        <div className="relative h-48 w-48 overflow-hidden rounded-xl border-2 border-border tft-glow-gold">
          <Image
      ...
      <div className="mb-6 grid grid-cols-2 gap-3">
        {question.choices.map((choice) => {
          ...
          let choiceStyle =
            "relative flex min-h-[56px] items-center justify-center rounded-xl border-2 px-4 py-3 text-center text-sm font-medium transition-all cursor-pointer select-none ";

          if (isFeedback) {
            if (isCorrect) {
              choiceStyle += "border-green-500 bg-green-500/20 text-green-300";
            } else if (isSelected && !isCorrect) {
              choiceStyle += "border-red-500 bg-red-500/20 text-red-300";
            } else {
              choiceStyle += "border-border bg-muted/20 text-muted-foreground opacity-50";
            }
          } else {
            if (isSelected) {
              choiceStyle += "border-primary bg-primary/10 tft-glow-gold";
            } else {
              choiceStyle += "border-border bg-card hover:border-primary/50 hover:bg-primary/5";
            }
          }
          ...
          return (
            ...
            >
              {isFeedback && isCorrect && (
                <CheckCircle2 className="absolute left-2 top-2 h-4 w-4 text-green-400" />
              )}
              {isFeedback && isSelected && !isCorrect && (
                <XCircle className="absolute left-2 top-2 h-4 w-4 text-red-400" />
              )}
              {choice.text}
            </button>
          );
        ...
        >
          {lastAnswer?.correct ? (
            <span>
              ...
              {state.combo >= 3 && ` (×${getComboMultiplier(state.combo)} 콤보)`}
            </span>
          ) : (
            <span>
              오답. 정답:{" "}
      ...
      {state.quizState === "answering" ? (
        <Button
          className={cn("w-full tft-gold-bg text-black font-bold")}
          size="lg"
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-12 text-center">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">{category.display_name} 퀴즈 결과</p>
      </div>

      <div className="mb-6 rounded-2xl border-2 tft-border-gold tft-glow-gold bg-card p-6">
